						...
						{messages.map((message) => (
							<div
								...
								className={`flex items-start space-x-3 mb-4 ${
									session?.user.id === message.user_id ? 'flex-row-reverse space-x-reverse' : ''
								}`}
							>
								<img
								...
								>
									<div>
										<span className='text-sm text-gray-500'>{message.user_name}</span>
					...
					<form onSubmit={handleSendMessage} className='p-4 border-t border-gray-200'>
						<div className='flex space-x-2'>
							<input
								...
								placeholder={session ? 'Type your message...' : 'Please login to chat'}
								disabled={!session}
